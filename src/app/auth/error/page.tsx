...
interface AuthErrorPageProps {
  searchParams: Promise<{ error?: string; error_description?: string; [key: string]: string | undefined }>
}
...
export default async function AuthErrorPage({ searchParams }: AuthErrorPageProps) {
  ...
  const allParams = Object.entries(params).filter(([key]) => key !== 'error' && key !== 'error_description')
          ...
          <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
            ...
            <div className="mt-2 text-xs text-red-600 space-y-1">
              ...
              <div className="mt-2 p-2 bg-red-100 rounded text-xs font-mono break-all">
                <strong>すべてのURLパラメータ ({allParams.length}個):</strong>
                <br />
                {allParams.length > 0 ? (
                  allParams.map(([key, value]) => (
                    <div key={key} className="mt-1">
                      <strong>{key}:</strong> {value || 'なし'}
                    </div>
                  ))
                ) : (
